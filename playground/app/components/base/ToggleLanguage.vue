...
async function toggleLocales() {
  ...
  const newLocale = locales[(locales.indexOf(locale.value) + 1) % locales.length] as 'en' | 'zh-CN'
...
<template>
  <button
    shadow
    :class="isChinese ? 'bg-blue-900' : 'bg-yellow-400'"
  ...
  >
    ...
    <span
      ...
      dark="text-gray-700"
      :class="isChinese ? 'translate-x-9' : 'translate-x-0'"
      inline-flex
      justify="center" items="center"
    >
      <div i="ri-translate" :class="isChinese ? '-scale-x-100' : ''" />
    </span>
  </button>
